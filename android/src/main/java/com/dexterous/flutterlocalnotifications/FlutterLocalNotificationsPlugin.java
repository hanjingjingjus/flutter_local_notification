...
import com.dexterous.flutterlocalnotifications.workers.ScheduleNotificationWorker;
...
public class FlutterLocalNotificationsPlugin implements MethodCallHandler, PluginRegistry.NewIntentListener, FlutterPlugin, ActivityAware {
    ...
    private static final String SHOW_CUSTOM_DAILY_AT_TIME_METHOD = "showCustomDailyAtTime";
    ...
    static String REPEAT_CUSTOM_NUM = "repeat_custom_num";
    static Gson gson;
...
//                repeatNotification(context, scheduledNotification, false);
                repeatNotificationByWorkManager(context, scheduledNotification, false);
            }
        }
    }
    ...
    //WorkManager创建延时任务
    private static void scheduleNotificationByWorkManager(Context context, final NotificationDetails notificationDetails, Boolean updateScheduledNotificationsCache) {
        Gson gson = buildGson();
        String notificationDetailsJson = gson.toJson(notificationDetails);
        Intent notificationIntent = new Intent(context, ScheduledNotificationReceiver.class);
        notificationIntent.putExtra(NOTIFICATION_DETAILS, notificationDetailsJson);
        ...
        Log.e("WorkManager创建延时任务", notificationDetails.millisecondsSinceEpoch + "," + System.currentTimeMillis());

        //单次执行任务
        Data data = new Data.Builder().putString("notificationDetailsJson", notificationDetailsJson).build();
        OneTimeWorkRequest oneTimeWorkRequest =
                new OneTimeWorkRequest.Builder(ScheduleNotificationWorker.class)
                        .setInputData(data)
                        .addTag(notificationDetails.id.toString())
                        .setInitialDelay(notificationDetails.millisecondsSinceEpoch - System.currentTimeMillis(), TimeUnit.MILLISECONDS)
                        .build();
    ...
    private static void repeatNotificationByWorkManager(Context context, NotificationDetails notificationDetails, Boolean updateScheduledNotificationsCache) {
        Gson gson = buildGson();
        String notificationDetailsJson = gson.toJson(notificationDetails);
        Intent notificationIntent = new Intent(context, ScheduledNotificationReceiver.class);
        notificationIntent.putExtra(NOTIFICATION_DETAILS, notificationDetailsJson);
        ...
        switch (notificationDetails.repeatInterval) {
            ...
            case Weekly:
                ...
                break;
            case CustomDaily:
                repeatInterval = 60000 * 60 * 24 * notificationDetails.repeatIntervalDay;
                break;
            default:
                break;
        }
    ...
    public void onMethodCall(MethodCall call, Result result) {
        switch (call.method) {
            ...
            case SHOW_CUSTOM_DAILY_AT_TIME_METHOD:
...
//                cancelAllNotifications(result);
                cancelAllNotificationsWorkManager(result);
                break;
...
//        cancelNotification(id);
        cancelNotificationWorkManager(id);
        result.success(null);
    }
...
//            repeatNotification(applicationContext, notificationDetails, true);
            repeatNotificationByWorkManager(applicationContext, notificationDetails, true);
            result.success(null);
        }
    }
...
//            scheduleNotification(applicationContext, notificationDetails, true);
            scheduleNotificationByWorkManager(applicationContext, notificationDetails, true);
            result.success(null);
        }
    }
    ...
    private void cancelNotificationWorkManager(Integer id) {
        WorkManager.getInstance(applicationContext).cancelUniqueWork(id.toString());
    ...
    private void cancelAllNotificationsWorkManager(Result result) {
        NotificationManagerCompat notificationManager = getNotificationManager(applicationContext);
        ...
        for (NotificationDetails scheduledNotification :
            ...
            WorkManager.getInstance(applicationContext).cancelUniqueWork(scheduledNotification.id.toString());
